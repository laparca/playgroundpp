...
#else
constexpr uint64_t abs_sub(const uint64_t a, const uint64_t b) { return (a > b) ? (a - b) : (b - a); }
...
constexpr uint64_t coord_x_from_pos(const uint64_t pos) { return (15 - pos) % 4; }

constexpr uint64_t coord_y_from_pos(const uint64_t pos) { return (15 - pos) / 4; }

constexpr uint64_t dest_coord_x(const uint64_t coord_value) { return coord_value % 4; }

constexpr uint64_t dest_coord_y(const uint64_t coord_value) { return coord_value / 4; }
#endif
...
constexpr int calc_number_of_inversions(uint64_t i, uint64_t j, uint64_t puz) {
  return i < 16 ? (j < 16 ? (inversion_cost_for(i, j, puz) + calc_number_of_inversions(i, j + 1, puz)) : calc_number_of_inversions(i + 1, i + 2, puz)) : 0;
}
...
constexpr int get_row_of_0(uint64_t p, uint64_t puz) { return p > 15 ? 0 : ((puz & 0xfull) == 0 ? p / 4 : get_row_of_0(p + 1, puz >> 4)); }
...
template <uint64_t Puz> constexpr uint64_t ManhattanDistance_helper<Puz, 16> = 0;

template <uint64_t Puz> constexpr uint64_t ManhattanDistance = ManhattanDistance_helper<Puz, 0>;
 ...
 */
template <Movement M> using Mov = std::integral_constant<Movement, M>;

template <class Puz, uint64_t Distance, uint64_t Moves, class Previous, class Movement> struct GameState {
  using puz = Puz;
 ...
 */
template <class Puz> using initial_GameState = tlist_add_t<GameState<Puz, ManhattanDistance<Puz::value>, 0, Nothing, Nothing>, EmptyList>;
 ...
 */
template <class Puz> using ProcList = tlist_add_t<initial_GameState<Puz>, EmptyList>;
...
template <class GS> struct generate_moves {
  template <class M> struct func {
...
template <class Puz> using generate_moves_t = typename generate_moves<Puz>::type;

template <class State1, class State2> struct GameStateSortFunc {
  static constexpr bool value = (State1::distance + State1::moves) < (State2::distance + State2::moves);
};

template <class State1> struct GameStateSortFunc<State1, Nothing> {
  static constexpr bool value = true;
};

/* The heuristic function we will use is the Mnahattan distance of the movement
 * and the length of all the moves we did */
template <class Puz, class Moves> constexpr uint64_t Heuristic = ManhattanDistance<Puz::value> + tlist_size_v<Moves>;

template <class ProcList> struct resolv;

template <class ProcList> using resolv_t = typename resolv<ProcList>::type;

template <bool IsComplete, class ProcList> struct resolv_helper;
template <bool IsComplete, class ProcList> using resolv_helper_t = resolv_helper<IsComplete, ProcList>;

template <class ProcList> struct resolv_helper<true, ProcList> {
  using type = tlist_head_t<ProcList>;
};

template <class ProcList> struct resolv_helper<false, ProcList> {
  using actGS = tlist_head_t<ProcList>;

  template <class List, class GS> using gs_add = tlist_sort_add_unique<GS, List, GameStateSortFunc>;

  using type = resolv_t<tlist_foldl_t<gs_add, tlist_tail_t<ProcList>, generate_moves_t<actGS>>>;
};

template <class ProcList> struct resolv : resolv_helper<std::is_same_v<typename tlist_head_t<ProcList>::puz, PuzzleResolved>, ProcList> {};

template <bool Solvable, class Puzzle> struct try_resolve_helper {
  using type = resolv_t<initial_GameState<Puzzle>>;
};

template <class Puzzle> struct try_resolve_helper<false, Puzzle> {
  using type = Nothing;
};

template <class Puzzle> struct try_resolve {
  using type = typename try_resolve_helper<is_solvable(Puzzle::value), Puzzle>::type;
};

#include <iostream>

constexpr char direction[4] = {[Up] = 'u', [Right] = 'r', [Down] = 'd', [Left] = 'l'};

template <class Path> struct show_path : show_path<typename Path::previous> {
  show_path() {
...
template <> struct show_path<Nothing> {
  show_path() { std::cout << std::endl; }
};
