...
TEST(chanel0_test, close_with_consumer_waiting) {
  laparca::chanel0<size_t> c;
  std::thread th{[&c] { ASSERT_FALSE(c.pop().has_value()); }};
...
TEST(chanel0_test, close_with_producer_waiting) {
  laparca::chanel0<size_t> c;
  std::thread th{[&c] { ASSERT_FALSE(c.push(1)); }};
...
TEST(chanel0_test, normal_behaviour) {
  ...
  std::thread producer{[&c] { ASSERT_TRUE(c.push(42)); }};
...
constexpr size_t sum_of_serie(size_t first, size_t last) { return (last + first) * (last - first + 1) / 2; }
